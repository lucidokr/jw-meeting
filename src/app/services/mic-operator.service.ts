...
@Injectable()
export class MicOperatorService {
  url:string = "/micoperator";
  constructor(
  ...
  add(brother: Brother): Observable<string> {
    return this.http.post(environment.url + this.url +"/"+brother._id, brother.micOperator, null)
  }

  edit(brother: Brother): Observable<string> {
    return this.http.put(environment.url + this.url +"/"+brother._id, brother.micOperator, null)
  }
