...
@Injectable()
export class AcousticsService {
  url:string = "/acoustics";
  constructor(
  ...
  add(brother: Brother): Observable<string> {
    return this.http.post(environment.url + this.url +"/"+brother._id, brother.acoustics, null)
  }

  edit(brother: Brother): Observable<string> {
    return this.http.put(environment.url + this.url +"/"+brother._id, brother.acoustics, null)
  }
